...
hal_result_t driver_destroy(){
	...
	for(i=0;i<max_switches;++i){
		if(switch_list[i] != NULL){
			driver_destroy_switch_by_dpid(switch_list[i]->dpid);
		}
	}
...
hal_result_t driver_connect_switches(uint64_t dpid_lsi1, switch_port_snapshot_t** port1, uint64_t dpid_lsi2, switch_port_snapshot_t** port2){
	...
	//Attach both ports
	if(driver_attach_port_to_switch(dpid_lsi1, vport1->of_port_state->name, &port_num) != HAL_SUCCESS){
		assert(0);
		return HAL_FAILURE;
	}
	port_num=0;
	if(driver_attach_port_to_switch(dpid_lsi2, vport2->of_port_state->name, &port_num) != HAL_SUCCESS){
		assert(0);
		return HAL_FAILURE;
	}
	...
	if(driver_bring_port_up(vport1->of_port_state->name) != HAL_SUCCESS || driver_bring_port_up(vport2->of_port_state->name) != HAL_SUCCESS){
