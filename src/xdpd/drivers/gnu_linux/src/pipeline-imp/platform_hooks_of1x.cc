...
rofl_result_t platform_post_init_of1x_switch(of1x_switch_t* sw){
	...
	ls_int->pkt_in_queue = new circular_queue<datapacket_t>(LSI_PKT_IN_QUEUE_SLOTS);
	ls_int->storage = new datapacket_storage( LSI_PKT_IN_STORAGE_MAX_BUF, LSI_PKT_IN_STORAGE_EXPIRATION_S); // todo make this value configurable
...
void platform_of1x_packet_in(const of1x_switch_t* sw, uint8_t table_id, datapacket_t* pkt, uint16_t send_len, of_packet_in_reason_t reason)
{
	...
	ROFL_DEBUG(DEFAULT, DRIVER_NAME" Enqueuing PKT_IN event for packet(%p) in switch: %s\n",pkt,sw->name);
	...
	if( ls_state->pkt_in_queue->non_blocking_write(pkt) == ROFL_SUCCESS ){
		...
		ROFL_DEBUG(DEFAULT, DRIVER_NAME" PKT_IN for packet(%p) could not be sent for sw:%s (PKT_IN queue full). Dropping..\n",pkt,sw->name);
