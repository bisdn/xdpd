...
void runPEX::init()
{
	...
	std::string pexName1 = "2_example";
	std::string port1 = "2_example_1";
	std::string port2 = "2_example_2";
		
	//Create two PEX
	try
	{
		pex_manager::create_pex_port(pexName1,port1,DPDK_SECONDARY);
		pex_manager::create_pex_port(pexName1,port2,DPDK_SECONDARY);
	
/*		pex_manager::create_pex_port(pexName1,pexName1,DPDK_KNI);
		pex_manager::create_pex_port(pexName2,pexName2,DPDK_KNI);
		pex_manager::create_pex_port(pexName3,pexName3,DPDK_KNI);
		pex_manager::create_pex_port(pexName4,pexName4,DPDK_KNI);
		pex_manager::create_pex_port(pexName5,pexName5,DPDK_KNI);
		pex_manager::create_pex_port(pexName6,pexName6,DPDK_KNI);
		pex_manager::create_pex_port(pexName7,pexName7,DPDK_KNI);
		pex_manager::create_pex_port(pexName8,pexName8,DPDK_KNI);
		pex_manager::create_pex_port(pexName9,pexName9,DPDK_KNI);
		pex_manager::create_pex_port(pexName10,pexName10,DPDK_KNI);*/
	}catch(...)
	{
		...
		*/
		...
		if(LSIs.size() != 0)
		{
			...
			try
			{
				...
				port_manager::attach_port_to_switch(dpid, port1, &port_number);
				port_number = 0;
				ROFL_INFO("[xdpd]["PLUGIN_NAME"] Attaching PEX port '%s' to LSI '%x'\n",port2.c_str(),dpid);	
				port_manager::attach_port_to_switch(dpid, port2, &port_number);
/*				port_number = 0;
				ROFL_INFO("[xdpd]["PLUGIN_NAME"] Attaching PEX port '%s' to LSI '%x'\n",pexName2.c_str(),dpid);	
