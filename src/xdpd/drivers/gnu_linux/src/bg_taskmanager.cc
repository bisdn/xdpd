...
int prepare_event_socket()
{
	...
	if ((sock = socket(PF_NETLINK, SOCK_RAW, NETLINK_ROUTE)) == -1){
		ROFL_ERR(DEFAULT, DRIVER_NAME" [bg] Couldn't open NETLINK_ROUTE socket, errno(%d): %s\n", errno, strerror(errno));
		return -1;
	}

	if(fcntl(sock, F_SETFL, fcntl(sock, F_GETFL) | O_NONBLOCK) < 0){
		// handle error
		ROFL_ERR(DEFAULT, DRIVER_NAME" [bg] Error fcntl, errno(%d): %s\n", errno, strerror(errno));
		return -1;
	}
	...
	if (bind(sock, (struct sockaddr *)&addr, sizeof(addr)) == -1){
		ROFL_ERR(DEFAULT, DRIVER_NAME" [bg] couldn't bind, errno(%d): %s\n", errno, strerror(errno));
		return -1;
	}
...
static rofl_result_t read_netlink_message(int fd){
	...
	for (; NLMSG_OK(nlh, (unsigned int)len); nlh = NLMSG_NEXT(nlh, len)) {
	    ...
	    if (nlh->nlmsg_type == RTM_NEWLINK || nlh->nlmsg_type == RTM_DELLINK){
				...
				ROFL_DEBUG_VERBOSE(DEFAULT, DRIVER_NAME" [bg] Interface changed status %s (%u) flags=0x%x change=0x%x\n",name, ifi->ifi_index, ifi->ifi_flags, ifi->ifi_change);
...
#ifdef DEBUG
		dummy++;
		//ROFL_DEBUG_VERBOSE(DEFAULT, DRIVER_NAME" Checking flow entries expirations %lu:%lu\n",now.tv_sec,now.tv_usec);
#endif
...
#ifdef DEBUG
		//ROFL_ERR(DEFAULT, DRIVER_NAME" Checking pool buffers expirations %lu:%lu\n",now.tv_sec,now.tv_usec);
#endif
...
void* x86_background_tasks_routine(void* param)
{
	...
	if(efd == -1){
		ROFL_ERR(DEFAULT, DRIVER_NAME" [bg] Error in epoll_create1, errno(%d): %s\n", errno, strerror(errno) );
		return NULL;
	}
	...
	if(epoll_ctl(efd,EPOLL_CTL_ADD,events_socket,&epe_port)==-1){
		ROFL_ERR(DEFAULT, DRIVER_NAME" [bg] Error in epoll_ctl, errno(%d): %s\n", errno, strerror(errno));
		return NULL;
	}
	...
	while(bg_continue_execution){
		...
		if(nfds==-1){
			//ROFL_DEBUG(DEFAULT, DRIVER_NAME" Epoll Failed\n");
			continue;
		}
		...
		for(i=0;i<nfds;i++){
			...
			if( (event_list[i].events & EPOLLERR) || (event_list[i].events & EPOLLHUP)/*||(event_list[i].events & EPOLLIN)*/){
				//error on this fd
				//ROFL_ERR(DEFAULT, DRIVER_NAME" Error in file descriptor\n");
