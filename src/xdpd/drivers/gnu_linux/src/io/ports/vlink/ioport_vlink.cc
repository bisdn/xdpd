...
void ioport_vlink::enqueue_packet(datapacket_t* pkt, unsigned int q_id){
	...
	if ( likely(of_port_state->up) && 
		...
		if( unlikely(q_id >= get_num_of_queues()) ){
			ROFL_DEBUG(DEFAULT, DRIVER_NAME"[vlink:%s] Packet(%p) trying to be enqueued in an invalid q_id: %u\n",  of_port_state->name, pkt, q_id);
			q_id = 0;
		...
		if(output_queues[q_id]->non_blocking_write(pkt) != ROFL_SUCCESS){
			ROFL_DEBUG(DEFAULT, DRIVER_NAME"[vlink:%s] Packet(%p) dropped. Congestion in output queue: %d\n",  of_port_state->name, pkt, q_id);
			//Drop packet
			bufferpool::release_buffer(pkt);
		...
		ROFL_DEBUG_VERBOSE(DEFAULT, DRIVER_NAME"[vlink:%s] Packet(%p) enqueued, buffer size: %d\n",  of_port_state->name, pkt, output_queues[q_id]->size());
		...
		if(len < MIN_PKT_LEN){
			ROFL_ERR(DEFAULT, DRIVER_NAME"[vlink:%s] ERROR: attempt to send invalid packet size for packet(%p) scheduled for queue %u. Packet size: %u\n", of_port_state->name, pkt, q_id, len);
			assert(0);
		}else{
			ROFL_DEBUG_VERBOSE(DEFAULT, DRIVER_NAME"[vlink:%s] dropped packet(%p) scheduled for queue %u\n", of_port_state->name, pkt, q_id);
		}
