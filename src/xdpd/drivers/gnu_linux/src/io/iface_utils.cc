...
rofl_result_t update_port_status(char * name){
	...
	if(update_physical_ports() != ROFL_SUCCESS){
		ROFL_ERR(DEFAULT, DRIVER_NAME"[ports] Update physical ports failed \n");
		assert(0);
	}
	...
	if ((rc = ioctl(sd, SIOCGIFINDEX, &ifr)) < 0){
		ROFL_ERR(DEFAULT, DRIVER_NAME"[ports] retrieval of interface index of port %s failed\n", name);
		return ROFL_FAILURE;
	}
...
static switch_port_t* fill_port(int sock, struct ifaddrs* ifa){
	...
	if (ioctl(sock, SIOCETHTOOL, &ifr)==-1){
		ROFL_WARN(DEFAULT, DRIVER_NAME"[ports] WARNING: unable to retrieve MAC address from iface %s via ioctl SIOCETHTOOL. Information will not be filled\n",ifr.ifr_name);
	}
	...
	ROFL_INFO(DEFAULT, DRIVER_NAME"[ports] Discovered interface %s mac_addr %02X:%02X:%02X:%02X:%02X:%02X \n",
...
rofl_result_t discover_physical_ports(){
	...
	for(ifa = ifaddr; ifa != NULL; ifa=ifa->ifa_next){
		...
		if( physical_switch_add_port(port) != ROFL_SUCCESS ){
			ROFL_ERR(DEFAULT, DRIVER_NAME"[ports] All physical port slots are occupied\n");
			freeifaddrs(ifaddr);
			assert(0);
			return ROFL_FAILURE;
		}
	...
	for(i=0; i<max_ports ; i++){
		if(array[i] != NULL){
			...
			if(update_port_status(array[i]->name) != ROFL_SUCCESS){
				ROFL_ERR(DEFAULT, DRIVER_NAME"[ports] Unable to retrieve link and/or admin status of the interface\n");
				freeifaddrs(ifaddr);
				assert(0);
				return ROFL_FAILURE;
			}
...
rofl_result_t create_virtual_port_pair(of_switch_t* lsw1, ioport** vport1, of_switch_t* lsw2, ioport** vport2){
	...
	if(!port1 || !port2){
		free(port1);
		assert(0);
		ROFL_ERR(DEFAULT, DRIVER_NAME"[ports] Not enough memory\n");
		return ROFL_FAILURE;
	}
	...
	*vport2 = new ioport_vlink(port2);
	...
	if( physical_switch_add_port(port1) != ROFL_SUCCESS ){
		...
		delete *vport1;
		delete *vport2;
		ROFL_ERR(DEFAULT, DRIVER_NAME"[ports] Unable to add vlink port1 to the physical switch; out of slots?\n");
		assert(0);
		return ROFL_FAILURE;
	}
	if( physical_switch_add_port(port2) != ROFL_SUCCESS ){
		...
		delete *vport1;
		delete *vport2;
		ROFL_ERR(DEFAULT, DRIVER_NAME"[ports] Unable to add vlink port2 to the physical switch; out of slots?\n");
		assert(0);
		return ROFL_FAILURE;
	}
...
rofl_result_t update_physical_ports(){
	...
	ROFL_DEBUG_VERBOSE(DEFAULT, DRIVER_NAME"[ports] Trying to update the list of physical interfaces...\n");
	...
	for (std::map<std::string, switch_port_t*>::iterator it = pipeline_ifaces.begin(); it != pipeline_ifaces.end(); ++it){
		if (system_ifaces.find(it->first) == system_ifaces.end() ) {
			...
			ROFL_INFO(DEFAULT, DRIVER_NAME"[ports] Interface %s has been removed from the system. The interface will now be detached from any logical switch it is attached to (if any), and removed from the list of physical interfaces.\n", it->first.c_str());
			...
			if(port->attached_sw && (hal_driver_detach_port_from_switch(port->attached_sw->dpid, port->name) != HAL_SUCCESS) ){
				ROFL_WARN(DEFAULT, DRIVER_NAME"[ports] WARNING: unable to detach port %s from switch. This can lead to an unknown behaviour\n", it->first.c_str());
				assert(0);
			}
	...
	for (std::map<std::string, struct ifaddrs*>::iterator it = system_ifaces.begin(); it != system_ifaces.end(); ++it){
		...
		if(!port){
			//ROFL_ERR(DEFAULT, DRIVER_NAME"[ports] Unable to initialize newly discovered interface %s\n", it->first.c_str());
			continue;
		}

		//Adding the
		if( physical_switch_add_port(port) != ROFL_SUCCESS ){
			ROFL_ERR(DEFAULT, DRIVER_NAME"[ports] Unable to add port %s to physical switch. Not enough slots?\n", it->first.c_str());
			freeifaddrs(ifaddr);
			continue;
		}
	...
	ROFL_DEBUG_VERBOSE(DEFAULT, DRIVER_NAME"[ports] Update of interfaces done.\n");
