...
static void clone_pkt_contents(datapacket_t* src, datapacket_t* dst){
	...
	pack_dst->init(pack_src->get_buffer(), pack_src->get_buffer_length(), pack_src->lsw, pack_src->in_port, pack_src->in_phy_port, true, pack_src->get_buffering_status() == X86_DATAPACKET_BUFFERED_IN_USER_SPACE );
