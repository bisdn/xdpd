...
inline bool epoll_ioscheduler::process_port_rx(unsigned tid, ioport* port){
	...
	ROFL_DEBUG_VERBOSE(DEFAULT, DRIVER_NAME" Trying to read at port %s with %d\n", port->of_port_state->name, READ_BUCKETS_PP);
	...
	for(i=0; i<READ_BUCKETS_PP; ++i){
		...
		if(likely(pkt != NULL)){
			...
			ROFL_DEBUG_VERBOSE(DEFAULT, DRIVER_NAME"[%s] reading finished at: %d/%d\n", port->of_port_state->name, i, READ_BUCKETS_PP);
			break;
		}
	}
...
inline int epoll_ioscheduler::process_port_tx(ioport* port){
	...
	for(q_id=0; q_id < IO_IFACE_NUM_QUEUES; ++q_id){
		...
		ROFL_DEBUG_VERBOSE(DEFAULT, DRIVER_NAME"[%s] Trying to write at port queue: %d with n_buckets: %d.\n", port->of_port_state->name, q_id, n_buckets);
...
void* epoll_ioscheduler::process_io(void* grp){
	...
	ROFL_DEBUG(DEFAULT, DRIVER_NAME"[epoll_ioscheduler] Launching I/O RX thread on process id: %u(%u) for group %u\n", is_rx? "RX":"TX", syscall(SYS_gettid), pthread_self(), pg->id);
	ROFL_DEBUG_VERBOSE(DEFAULT, DRIVER_NAME"[epoll_ioscheduler] Initialization of epoll completed in thread:%d\n",pthread_self());
	...
	*/
	while(likely(iomanager::keep_on_working(pg))){
		...
		if(unlikely(res == -1)){
			...
			if(res == 0){
				...
				ROFL_DEBUG_VERBOSE(DEFAULT, DRIVER_NAME"[epoll_ioscheduler] Got %d events\n", res); 
				for(i=0; i<res; ++i){
	...
	ROFL_DEBUG(DEFAULT, DRIVER_NAME"[epoll_ioscheduler] Finishing execution of the %s I/O thread: #%u\n", is_rx? "RX":"TX", pthread_self());
