...
static void* process_packet_ins(void* param){
	...
	while(likely(keep_on_pktins)){
		...
		XDPD_DEBUG(DRIVER_NAME"[pktin_dispatcher] Processing PKT_IN for packet(%p), mbuf %p, switch %p\n", pkt, mbuf, sw);
		...
		if(unlikely(id == datapacket_storage::ERROR)){
			XDPD_DEBUG(DRIVER_NAME"[pktin_dispatcher] PKT_IN for packet(%p) could not be stored in the storage. Dropping..\n",pkt);
		...
		if( rv != HAL_SUCCESS ){
			XDPD_DEBUG(DRIVER_NAME"[pktin_dispatcher] PKT_IN for packet(%p) could not be sent to sw:%s controller. Dropping..\n",pkt,sw->name);
...
rofl_result_t pktin_dispatcher_init(){
	...
	if(!pkt_ins){
		XDPD_ERR(DRIVER_NAME"[pktin_dispatcher] Unable to create PKT_INs ring queue\n");
		return ROFL_FAILURE;
	}
	...
	if(pthread_create(&pktin_thread, NULL, process_packet_ins, NULL)<0){
		XDPD_ERR(DRIVER_NAME"[pktin_dispatcher] pthread_create failed, errno(%d): %s\n", errno, strerror(errno));
		return ROFL_FAILURE;
	}
