...
void tm_dump_measurements(void){
	...
	ROFL_INFO(DEFAULT, "Stage name \t\t\t| Average ticks | %% overall ticks(*) | # pkts\n");
	ROFL_INFO(DEFAULT, "*****************************************************************************\n");
...
void tm_dump_pkt(datapacket_t* pkt){
	...
	unsigned int i,j;	
	time_measurements_t* tm = &((xdpd::gnu_linux::datapacket_dpdk_t*)pkt->platform_state)->tm_state;
	
	ROFL_INFO(DEFAULT, "Dumping path of pkt (%p)", pkt);
	if(tm->pkt_out)
		ROFL_INFO(DEFAULT, " [PKT_OUT]");
	ROFL_INFO(DEFAULT, ":\n");
