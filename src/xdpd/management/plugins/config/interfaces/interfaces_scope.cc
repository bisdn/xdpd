	...
	if(setting.exists(BLACKLIST)){	
		for(int i=0; i<setting[BLACKLIST].getLength(); ++i){
			...
			if(port_manager::exists(port) == false){
				ROFL_ERR(DEFAULT, CONF_PLUGIN_ID "%s: attempting to blacklist an invalid port '%s'. Port does not exist!\n", setting.getPath().c_str(), port.c_str());
...
void virtual_ifaces_scope::post_validate(libconfig::Setting& setting, bool dry_run){
	...
	if(setting.getLength()%2 != 0){
		ROFL_ERR(DEFAULT, CONF_PLUGIN_ID "%s: malformed virtual interfaces section. There must be defined in pairs\n", setting.getPath().c_str());
		throw eConfParseError(); 	
	...
	//Detect existing subscopes (logical switches) and register
 	for(int i = 0; i<setting.getLength(); ++i){
		if( !setting[i].exists(VIF_LINK) || !setting[i].exists(VIF_LSI) ){
			ROFL_ERR(DEFAULT, CONF_PLUGIN_ID "%s: missing '%s' and/or '%s' mandatory parameters in vif '%s' configuration section.\n", setting.getPath().c_str(), VIF_LINK, VIF_LSI, setting[i].getName());
			throw eConfParseError(); 	
		...
		if((partial_links.find(key) == partial_links.end())){
			...
			if(provisioned_links.find(key) != provisioned_links.end()){
				ROFL_ERR(DEFAULT, CONF_PLUGIN_ID "%s: duplicated vlink '%s' name!\n", setting.getPath().c_str(), key.c_str());
				throw eConfParseError(); 
			...
			//Check self connection
 			if(lsi_name == value){
				ROFL_ERR(DEFAULT, CONF_PLUGIN_ID "%s: unable to create vlink. Switch '%s' cannot be connected to itself!\n", setting.getPath().c_str(), value.c_str());
				throw eConfParseError(); 
			}	
			...
			if(!dry_run){
				...
				try{
					...
					ROFL_ERR(DEFAULT, CONF_PLUGIN_ID "%s: unable to create vlink. Switch '%s' does not exist.\n", setting.getPath().c_str(), lsi_name.c_str());
					throw eConfParseError(); 	
	...
	if(partial_links.size() > 0){
		ROFL_ERR(DEFAULT, CONF_PLUGIN_ID "%s: error, some virtual interfaces could not be connected. This is likely due to one or more unpaired '%s' values.\n", setting.getPath().c_str(), VIF_LINK);
		throw eConfParseError(); 	
	}
}
