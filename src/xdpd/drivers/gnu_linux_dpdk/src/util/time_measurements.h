...
extern time_measurements_t global_measurements;
	...
	#define TM_INIT_PKT(pkt_p)\
		do{\
			tm_init_pkt(&((datapacket_dpdk_t*)pkt_p->platform_state)->tm_state);\
		}while(0)
	...
	#define TM_STAMP_STAGE(pkt_p, stage)\
		do{\
			...
			tm_stamp_stage(& ( ( (datapacket_dpdk_t*) pkt_p->platform_state ) ->tm_state ), stage);\
		}while(0)
	...
	#define TM_STAMP_PKT_OUT(pkt_p)\
		do{\
			((datapacket_dpdk_t*)pkt_p->platform_state)->tm_state.pkt_out = true;\
		}while(0)


	//PKT accumulation to general counters
	#define TM_AGGREGATE_PKT(pkt_p) tm_aggregate_pkt(& ( ( (datapacket_dpdk_t*) pkt_p->platform_state ) ->tm_state ))	
