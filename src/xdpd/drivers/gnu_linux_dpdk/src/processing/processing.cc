...
rofl_result_t processing_init(void){
	...
	ROFL_DEBUG(DEFAULT, DRIVER_NAME"[processing] Processing init: %u logical cores guessed from rte_eal_get_configuration(). Master is: %u\n", config->lcore_count, config->master_lcore);
	...
	for(i=0; i < RTE_MAX_LCORE; ++i){
		...
		if(role == ROLE_RTE){
			...
			if(i != config->master_lcore){
				...
				ROFL_DEBUG(DEFAULT, DRIVER_NAME"[processing] Marking core %u as available\n",i);
			}
			...
			if(direct_pools[sock_id] == NULL){
				...
				*/
				...
				ROFL_INFO(DEFAULT, DRIVER_NAME"[processing] Creating %s with #mbufs %u for CPU socket %u\n", pool_name, mbuf_pool_size, sock_id);
...
rofl_result_t processing_destroy(void){
	...
	ROFL_DEBUG(DEFAULT, DRIVER_NAME"[processing] Shutting down all active cores\n");
	...
	for(i=0;i<RTE_MAX_LCORE;++i){
		if(processing_core_tasks[i].available && processing_core_tasks[i].active){
			ROFL_DEBUG(DEFAULT, DRIVER_NAME"[processing] Shutting down active core %u\n",i);
...
rofl_result_t processing_schedule_port(switch_port_t* port){
	...
	switch(port->type){
		case PORT_TYPE_PHYSICAL: 
			if(total_num_of_phy_ports == PROCESSING_MAX_PORTS){
				ROFL_ERR(DEFAULT, DRIVER_NAME"[processing] Reached already PROCESSING_MAX_PORTS(%u). All cores are full. No available port slots\n", PROCESSING_MAX_PORTS);
	...
	if(lcore_sel == RTE_MAX_LCORE){
		ROFL_ERR(DEFAULT, DRIVER_NAME"[processing] ERROR: All cores are full. No available port slots\n");
		rte_spinlock_unlock(&mutex);
		return ROFL_FAILURE;
	}
	...
	if(port->type == PORT_TYPE_PHYSICAL){
		...
		if( (socket_id != 0xFFFFFFFF) && (socket_id != rte_lcore_to_socket_id(lcore_sel))){
			ROFL_ERR(DEFAULT, DRIVER_NAME"[processing] WARNING: The core selected %u[cpu socket %u] and the port %s(cpu socket: %u) are in different CPU sockets!\n This configuration is SUBOPTIMAL!! Consider using another coremask.\n",
				 lcore_sel, rte_lcore_to_socket_id(lcore_sel),
				 port->name, socket_id);
#ifdef ABORT_ON_UNMATCHED_SCHED
			ROFL_ERR(DEFAULT, DRIVER_NAME"[processing] ERROR: The core selected %u[cpu socket %u] and the port %s(cpu socket: %u) are in different CPU sockets!\n No available core .\n",
				 lcore_sel, rte_lcore_to_socket_id(lcore_sel),
				 port->name, socket_id);
	...
	ROFL_DEBUG(DEFAULT, DRIVER_NAME"[processing] Selected core %u for scheduling port %s(%p)\n", lcore_sel, port->name, port); 
	...
	if(processing_core_tasks[lcore_sel].port_list[*num_of_ports] != NULL){
		ROFL_ERR(DEFAULT, DRIVER_NAME"[processing] Corrupted state on the core task list\n");
		assert(0);
		rte_spinlock_unlock(&mutex);
		return ROFL_FAILURE;
	}
	...
	if(!processing_core_tasks[lcore_sel].active){
		...
		ROFL_DEBUG(DEFAULT, DRIVER_NAME"[processing] Launching core %u due to scheduling action of port %p\n", lcore_sel, port);

		//Launch
		ROFL_DEBUG_VERBOSE(DEFAULT, "Pre-launching core %u due to scheduling action of port %p\n", lcore_sel, port);
		...
		ROFL_DEBUG_VERBOSE(DEFAULT, "Post-launching core %u due to scheduling action of port %p\n", lcore_sel, port);
	}
...
rofl_result_t processing_deschedule_port(switch_port_t* port){
	...
	if(*scheduled == false){
		ROFL_ERR(DEFAULT, DRIVER_NAME"[processing] Tyring to descheduled an unscheduled port\n");
		assert(0);
		return ROFL_FAILURE;
	}
	...
	if(core_task->num_of_rx_ports == 0){
		if(rte_eal_get_lcore_state(*core_id) != RUNNING){
			ROFL_ERR(DEFAULT, DRIVER_NAME"[processing] Corrupted state; port was marked as active, but EAL informs it was not running..\n");
			assert(0);
			
		}
		
		ROFL_DEBUG(DEFAULT, DRIVER_NAME"[processing] Shutting down core %u, since port list is empty\n",i);
...
void processing_dump_core_states(void){
	...
	ROFL_INFO(DEFAULT, "%s", ss.str().c_str());
}
