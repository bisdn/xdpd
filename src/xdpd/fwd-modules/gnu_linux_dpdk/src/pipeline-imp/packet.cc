...
{
	datapacketx86 *pack = ((dpdk_pkt_platform_state_t*)pkt->platform_state)->pktx86;
...
void platform_packet_output(datapacket_t* pkt, switch_port_t* output_port){
	...
	if (NULL == (pack = ((dpdk_pkt_platform_state_t*)pkt->platform_state)->pktx86 )){
		assert(0);
		return;
	}
	...
	if(output_port == flood_meta_port || output_port == all_meta_port){ //We don't have STP, so it is the same
		...
		for(unsigned i=0;i<LOGICAL_SWITCH_MAX_LOG_PORTS;++i){
			...
			replica_pack = ((dpdk_pkt_platform_state_t*)pkt->platform_state)->pktx86;
