...
mmap_tx::mmap_tx(
		...
		tpos(0)
{
	XDPD_DEBUG_VERBOSE(DRIVER_NAME" mmap_tx(%p)::mmap_tx() %s\n",
			this, "RX-RING");
	...
	XDPD_DEBUG_VERBOSE(DRIVER_NAME" mmap_tx(%p)::initialize() block-size:%u block-nr:%u frame-size:%u frame-nr:%u\n",
			this,
			req.tp_block_size,
			req.tp_block_nr,
			req.tp_frame_size,
	...
	{
		XDPD_ERR(DRIVER_NAME" mmap_tx(%p)::initialize() setsockopt() sys-call failed for PACKET_VERSION "
			...
			(void *) &req, sizeof(req))) < 0)
	{
		...
		XDPD_DEBUG_VERBOSE(DRIVER_NAME" mmap_tx(%p)::initialize() setsockopt() sys-call failed "
				"rc: %d errno: %d (%s)\n", this, rc, errno, strerror(errno));
		throw eConstructorMmapTx();
	}
	...
	{
		XDPD_ERR(DRIVER_NAME" mmap_tx(%p)::initialize() Could not change buffer_size\n");
		throw eConstructorMmapTx();
	}
#endif
...
//	}
	...
	{
		XDPD_ERR(DRIVER_NAME" mmap_tx(%p)::initialize() mmap() sys-call failed "
				"rc: %d errno: %d (%s)\n", this, rc, errno, strerror(errno));
		throw eConstructorMmapTx();
	}
...
mmap_tx::~mmap_tx()
{
	...
	if (-1 != sd)
	{
		if (map != MAP_FAILED)
		{
			...
			{
				XDPD_ERR(DRIVER_NAME" mmap_tx(%p)::~mmap_tx() %s => errno: %d (%s) \n",
