...
void static_pktclassifier::dump(datapacket_t* pkt){
	...
	ROFL_DEBUG(DRIVER_NAME" [static_pktclassifier] datapacketx86(%p) soframe: %p framelen: %zu\n", this, get_buffer(pkt), get_buffer_length(pkt));


	//while (NULL != frame) {
	//	ROFL_ERR(DRIVER_NAME" [static_pktclassifier] %s\n", frame->c_str());
	//	frame = frame->next;
	//}
	...
	ROFL_DEBUG(DRIVER_NAME" [static_pktclassifier] content: %s\n", sstr.str().c_str());
