...
if test "$is_gxx" == "0"; then
	...
	AC_MSG_CHECKING(for g++ compatibility)
	...
	if test $num_dots -lt 2 ; then
		...
		AC_MSG_CHECKING(again for g++ compatibility)
	fi
	...
	if test "$buggy_gxx" = "no"; then
		...
		AC_MSG_RESULT(incompatible)
		AC_ERROR([ERROR: your g++ version '$gxx_version' is in the list of blacklisted g++ vesions {$BLACKLISTED_GXX_VERSIONS}. This is due to bugs affecting -O3 optimizations. Please use another version. ])
	fi
fi
