...
*/ 
rofl_result_t openflow12_switch::notify_port_attached(const switch_port_t* port){
	return endpoint->notify_port_attached(port);
}
rofl_result_t openflow12_switch::notify_port_detached(const switch_port_t* port){
...
*/
void openflow12_switch::rpc_connect_to_ctl(enum rofl::csocket::socket_type_t socket_type, caddress const& controller_addr){
	rofl::openflow::cofhello_elem_versionbitmap versionbitmap;
	versionbitmap.add_ofp_version(version);
	endpoint->rpc_connect_to_ctl(versionbitmap, 0, socket_type, controller_addr);
}

void openflow12_switch::rpc_connect_to_ctl(enum rofl::csocket::socket_type_t socket_type, cparams const& socket_params){
	rofl::openflow::cofhello_elem_versionbitmap versionbitmap;
	versionbitmap.add_ofp_version(version);
