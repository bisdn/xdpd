...
void port_manager::blacklist(std::string& port_name){
	...
	if(! exists(port_name)){
		XDPD_ERR("[xdpd][port_manager] ERROR: attempting to blacklist a non-existent port %s\n", port_name.c_str());
		throw ePmInvalidPort();
	}
	...
	if(is_attached(port_name)){
		XDPD_ERR("[xdpd][port_manager] ERROR: attempting to blacklist port %s which is attached to an LSI\n", port_name.c_str());
		throw ePmInvalidPort(); 	
	}
	...
	XDPD_INFO("[xdpd][port_manager] Port %s blacklisted\n", port_name.c_str());
}
...
void port_manager::whitelist(std::string& port_name){
	...
	//Check first if it exists
	if(! exists(port_name)){
		XDPD_ERR("[xdpd][port_manager] ERROR: attempting to whitelist a non-existent port %s\n", port_name.c_str());
		throw ePmInvalidPort();
	}
	...
	XDPD_INFO("[xdpd][port_manager] Port %s whitelisted\n", port_name.c_str());
}
...
void port_manager::bring_up(std::string& name){
	...
	XDPD_DEBUG("[xdpd][port_manager] Port %s brought administratively up\n", name.c_str());
}
...
void port_manager::bring_down(std::string& name){
	...
	//Redundant, the driver should inform us about the change of state in the port
	XDPD_DEBUG("[xdpd][port_manager] Port %s brought administratively down\n", name.c_str());
}
...
void port_manager::attach_port_to_switch(uint64_t dpid, std::string& port_name, unsigned int* of_port_num){
	...
	if(!exists(port_name)){
		pthread_mutex_unlock(&port_manager::mutex);
		XDPD_ERR("[xdpd][port_manager] ERROR: Attempting to attach a non-existent port %s to switch with dpid 0x%llx at port %u\n", port_name.c_str(), (long long unsigned)dpid, *of_port_num);
		throw ePmInvalidPort();
	}
	...
	if(!switch_manager::exists(dpid)){
		pthread_mutex_unlock(&port_manager::mutex);
		XDPD_ERR("[xdpd][port_manager] ERROR: Attempting to attach port %s to a non-existent switch with dpid 0x%llx at port %u\n", port_name.c_str(), (long long unsigned)dpid, *of_port_num);
	...
	if(hal_driver_attach_port_to_switch(dpid, port_name.c_str(), of_port_num) != HAL_SUCCESS){
		pthread_mutex_unlock(&port_manager::mutex);
		assert(0);
		XDPD_ERR("[xdpd][port_manager] ERROR: Driver was unable to attach port %s to switch with dpid 0x%llx at port %u\n", port_name.c_str(), (long long unsigned)dpid, *of_port_num);
		throw ePmUnknownError(); 
	}
	
	XDPD_INFO("[xdpd][port_manager] Port %s attached to switch with dpid 0x%llx at port %u\n", port_name.c_str(), (long long unsigned)dpid, *of_port_num);
	...
	if(port_snapshot == NULL ){
		assert(0);
		XDPD_ERR("[xdpd][port_manager] ERROR: Driver was unable to generate a port_snapshot for port %s during attachment; out of memory? The port may become unusable\n", port_name.c_str());
		//TODO: detach?
		pthread_mutex_unlock(&port_manager::mutex);
		throw ePmUnknownError(); 
	}
...
void port_manager::connect_switches(uint64_t dpid_lsi1, unsigned int* port_num1, std::string& port_name1, uint64_t dpid_lsi2, unsigned int* port_num2, std::string& port_name2) {
	...
	if(!switch_manager::exists(dpid_lsi1) || !switch_manager::exists(dpid_lsi2) ){
		pthread_mutex_unlock(&port_manager::mutex);
		XDPD_ERR("[xdpd][port_manager] ERROR: switch with dpid 0x%llx or 0x%llx (or both) do not exist.\n", (long long unsigned)dpid_lsi1, (long long unsigned)dpid_lsi2);
		throw eOfSmDoesNotExist();
	}

	if(hal_driver_connect_switches(dpid_lsi1, port_num1, &port1, dpid_lsi2, port_num2, &port2) != HAL_SUCCESS){
		pthread_mutex_unlock(&port_manager::mutex);
		XDPD_ERR("[xdpd][port_manager] Unknown ERROR: driver was unable to create a link between switch with dpid 0x%llx and 0x%llx\n", (long long unsigned)dpid_lsi1, (long long unsigned)dpid_lsi2);
		assert(0);
		throw ePmUnknownError(); 
	}
	...
	XDPD_INFO("[xdpd][port_manager] Link created between switch with dpid 0x%llx and 0x%llx, with virtual interface names %s and %s respectively \n", (long long unsigned)dpid_lsi1, (long long unsigned)dpid_lsi2, port1->name, port2->name);
...
void port_manager::detach_port_from_switch(uint64_t dpid, std::string& port_name){
	...
	if(!port_snapshot){
		pthread_mutex_unlock(&port_manager::mutex);
		XDPD_ERR("[xdpd][port_manager] ERROR: Attempting to detach non-existent port %s from switch with dpid 0x%llx\n", port_name.c_str(), (long long unsigned)dpid);
		assert(0);
		throw ePmInvalidPort();
	}
	...
	if(port_snapshot->attached_sw_dpid != dpid){
		pthread_mutex_unlock(&port_manager::mutex);
		switch_port_destroy_snapshot(port_snapshot);	
		XDPD_ERR("[xdpd][port_manager] ERROR: Attempting to detach port %s, which is not attached to switch with dpid 0x%llx\n", port_name.c_str(), (long long unsigned)dpid);
	...
	if(hal_driver_detach_port_from_switch(dpid,port_name.c_str()) != HAL_SUCCESS){
		pthread_mutex_unlock(&port_manager::mutex);
		switch_port_destroy_snapshot(port_snapshot);	
		XDPD_ERR("[xdpd][port_manager] Unknown ERROR: Driver was unable to detach non-existent port %s from switch with dpid 0x%llx\n", port_name.c_str(), (long long unsigned)dpid);
		assert(0);	
		throw ePmUnknownError();
	}

	XDPD_INFO("[xdpd][port_manager] Port %s detached from switch with dpid 0x%llx\n", port_name.c_str(), (long long unsigned)dpid);
...
void port_manager::detach_port_from_switch_by_num(uint64_t dpid, unsigned int port_num){
	...
	if(!port_snapshot){
		pthread_mutex_unlock(&port_manager::mutex);
		XDPD_ERR("[xdpd][port_manager] ERROR: Attempting to detach port number %u from switch with dpid 0x%llx which is invalid\n", (long long unsigned)dpid, port_num);
		assert(0);
		throw ePmInvalidPort();
	}
	...
	if(port_snapshot->attached_sw_dpid != dpid){
		pthread_mutex_unlock(&port_manager::mutex);
		switch_port_destroy_snapshot(port_snapshot);	
		XDPD_ERR("[xdpd][port_manager] ERROR: Attempting to detach port number %u from switch with dpid 0x%llx which does not exist\n", (long long unsigned)dpid, port_num);
		throw ePmPortNotAttachedError();	
	}
	...
	if(hal_driver_detach_port_from_switch_at_port_num(dpid,port_num) != HAL_SUCCESS){
		pthread_mutex_unlock(&port_manager::mutex);
		XDPD_ERR("[xdpd][port_manager] Unknown ERROR: Driver was unabel to detach port number %u from switch with dpid 0x%llx\n", port_num, (long long unsigned)dpid);
		assert(0);
		switch_port_destroy_snapshot(port_snapshot);	
		throw eOfSmGeneralError();
	}

	XDPD_INFO("[xdpd][port_manager] Port %d detached from switch with dpid 0x%llx\n", port_num, (long long unsigned)dpid);
