		...
		rpos(0)
{
	...
	ROFL_DEBUG_VERBOSE(DEFAULT, DRIVER_NAME" mmap_rx(%p)::mmap_rx() %s\n",
			this, "RX-RING");
	...
	ROFL_DEBUG_VERBOSE(DEFAULT, DRIVER_NAME" mmap_rx(%p)::initialize() block-size:%u block-nr:%u frame-size:%u frame-nr:%u\n",
			this,
			req.tp_block_size,
			req.tp_block_nr,
			req.tp_frame_size,
	...
	{
		ROFL_ERR(DEFAULT, DRIVER_NAME" mmap_rx(%p)::initialize() setsockopt() sys-call failed for PACKET_VERSION "
			...
			(void *) &req, sizeof(req))) < 0)
	{
		...
		ROFL_DEBUG_VERBOSE(DEFAULT, DRIVER_NAME" mmap_rx(%p)::initialize() setsockopt() sys-call failed "
				"rc: %d errno: %d (%s)\n", this, rc, errno, strerror(errno));
		throw eConstructorMmapRx();	
	}
...
//	}
	...
	{
		ROFL_ERR(DEFAULT, DRIVER_NAME" mmap_rx(%p)::initialize() mmap() sys-call failed "
				"rc: %d errno: %d (%s)\n", this, rc, errno, strerror(errno));
		throw eConstructorMmapRx();	
	}
...
mmap_rx::~mmap_rx()
{
	...
	if (-1 != sd)
	{
		if (map != MAP_FAILED)
		{
			...
			{
				ROFL_ERR(DEFAULT, DRIVER_NAME" mmap_rx(%p)::~mmap_rx() %s => errno: %d (%s) \n",
