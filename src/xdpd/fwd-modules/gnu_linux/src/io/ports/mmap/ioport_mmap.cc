...
void ioport_mmap::enqueue_packet(datapacket_t* pkt, unsigned int q_id){
	...
	if ( likely(of_port_state->up) && 
		...
		if( unlikely(q_id >= get_num_of_queues()) ){
			ROFL_DEBUG(FWD_MOD_NAME"[mmap:%s] Packet(%p) trying to be enqueued in an invalid q_id: %u\n",  of_port_state->name, pkt, q_id);
			q_id = 0;
		...
		if(output_queues[q_id]->non_blocking_write(pkt) != ROFL_SUCCESS){
			...
			ROFL_DEBUG(FWD_MOD_NAME"[mmap:%s] Packet(%p) dropped. Congestion in output queue: %d\n",  of_port_state->name, pkt, q_id);
			//Drop packet
			bufferpool::release_buffer(pkt);
		...
		ROFL_DEBUG_VERBOSE(FWD_MOD_NAME"[mmap:%s] Packet(%p) enqueued, buffer size: %d\n",  of_port_state->name, pkt, output_queues[q_id]->size());
		...
		if(len < MIN_PKT_LEN){
			ROFL_ERR(FWD_MOD_NAME"[mmap:%s] ERROR: attempt to send invalid packet size for packet(%p) scheduled for queue %u. Packet size: %u\n", of_port_state->name, pkt, q_id, len);
			assert(0);
		}else{
			ROFL_DEBUG_VERBOSE(FWD_MOD_NAME"[mmap:%s] dropped packet(%p) scheduled for queue %u\n", of_port_state->name, pkt, q_id);
		}
...
next:
	...
	if ( unlikely(hdr->tp_mac + hdr->tp_snaplen > rx->get_tpacket_req()->tp_frame_size) ) {
		ROFL_DEBUG_VERBOSE(FWD_MOD_NAME"[mmap:%s] sanity check during read mmap failed\n",of_port_state->name);
	...
	if (PACKET_OUTGOING == sll->sll_pkttype) {
		/*ROFL_DEBUG_VERBOSE(FWD_MOD_NAME" cioport(%s)::handle_revent() outgoing "
...
#if 0
	ROFL_DEBUG_VERBOSE(FWD_MOD_NAME" %s(): datapacketx86 %p to tpacket_hdr %p\n"
			"	data = %p\n,"
...
unsigned int ioport_mmap::write(unsigned int q_id, unsigned int num_of_buckets){
	...
	for ( ; 0 < num_of_buckets; --num_of_buckets ) {
		...
		//Check
		if(queue->size() == 0){
			ROFL_DEBUG_VERBOSE(FWD_MOD_NAME"[mmap:%s] no packet left in output_queue %u left, %u buckets left\n",
					of_port_state->name,
					q_id,
					num_of_buckets);
			break;
		...
		if(!pkt){
			ROFL_ERR(FWD_MOD_NAME"[mmap:%s] A packet has been discarded due to race condition on the output queue. Are you really running the TX group with a single thread? output_queue %u left, %u buckets left\n",
				of_port_state->name,
				q_id,
				num_of_buckets);
		...
		if(unlikely(pkt_x86->get_buffer_length() > mps)){
			...
			ROFL_ERR(FWD_MOD_NAME"[mmap:%s] Packet length above the Max Packet Size (MPS). Packet length: %u, MPS %u.. discarding\n", of_port_state->name, pkt_x86->get_buffer_length(), mps);
			assert(0);
	...
	if (likely(cnt > 0)) {
		ROFL_DEBUG_VERBOSE(FWD_MOD_NAME"[mmap:%s] schedule %u packet(s) to be send\n", __FUNCTION__, cnt);

		// send packets in TX
		if(unlikely(tx->send() != ROFL_SUCCESS)){
			ROFL_ERR(FWD_MOD_NAME"[mmap:%s] ERROR while sending packets. This is due very likely to an invalid ETH_TYPE value. Now the port will be reset in order to continue operation\n", of_port_state->name);
			assert(0);
			...
			*/
			...
			if(!pg){
				assert(0);
				ROFL_DEBUG(FWD_MOD_NAME"[mmap:%s] ERROR: Unable to update port-group hash. The port might be left unusable\n", of_port_state->name);
			}else
				ROFL_DEBUG(FWD_MOD_NAME"[mmap:%s] Port reset was successful\n", of_port_state->name);
			pg->running_hash++;
...
rofl_result_t ioport_mmap::up() {
	...
	*/
	...
	if (ioctl(sd, SIOCETHTOOL, &ifr) < 0) {
		ROFL_WARN(FWD_MOD_NAME"[mmap:%s] Unable to detect if the Generic Receive Offload (GRO) feature on the NIC is enabled or not. Please make sure it is disabled using ethtool or similar...\n", of_port_state->name);
		
	}else{
		//Show nice messages in debug mode
		if(eval.data == 0){
			ROFL_DEBUG(FWD_MOD_NAME"[mmap:%s] GRO already disabled.\n", of_port_state->name);
		}else{
			//Do it
			...
			if (ioctl(sd, SIOCETHTOOL, &ifr) < 0) {
				ROFL_ERR(FWD_MOD_NAME"[mmap:%s] Could not disable Generic Receive Offload feature on the NIC. This can be potentially dangeros...be advised!\n",  of_port_state->name);
			}else{
				ROFL_DEBUG(FWD_MOD_NAME"[mmap:%s] GRO successfully disabled.\n", of_port_state->name);
			}
	...
	if (ioctl(sd, SIOCETHTOOL, &ifr) < 0) {
		ROFL_WARN(FWD_MOD_NAME"[mmap:%s] Unable to detect if the Large Receive Offload (LRO) feature on the NIC is enabled or not. Please make sure it is disabled using ethtool or similar...\n", of_port_state->name);
	} else {
		if ((eval.data & ETH_FLAG_LRO) == 0) {
			//Show nice messages in debug mode
			ROFL_DEBUG(FWD_MOD_NAME"[mmap:%s] LRO already disabled.\n", of_port_state->name);
		} else {
			//Do it
			...
			if (ioctl(sd, SIOCETHTOOL, &ifr) < 0)
				ROFL_ERR(FWD_MOD_NAME"[mmap:%s] Could not disable Large Receive Offload (LRO) feature on the NIC. This can be potentially dangeros...be advised!\n",  of_port_state->name);
			else
				ROFL_DEBUG(FWD_MOD_NAME"[mmap:%s] LRO successfully disabled.\n", of_port_state->name);
		}
	}
	...
	if(ioctl(sd, SIOCGIFMTU, &ifr) < 0) {
		ROFL_ERR(FWD_MOD_NAME"[mmap:%s] Could not retreive MTU value from NIC. Default %u Max Packet Size(MPS) size will be used (%u total bytes). Packets exceeding this size will be DROPPED (Jumbo frames).\n",  of_port_state->name, (PORT_DEFAULT_PKT_SIZE-PORT_ETHER_LENGTH), PORT_DEFAULT_PKT_SIZE);
		mps = PORT_DEFAULT_PKT_SIZE;	
	}else{
		mps = ifr.ifr_mtu+PORT_ETHER_LENGTH;
		ROFL_DEBUG(FWD_MOD_NAME"[mmap:%s] Discovered Max Packet Size(MPS) of %u.\n",  of_port_state->name, mps);
	}
	...
	if (IFF_UP & ifr.ifr_flags){
		...
		if(!rx){	
			ROFL_DEBUG_VERBOSE(FWD_MOD_NAME"[mmap:%s] generating a new mmap_rx for RX\n",of_port_state->name);
			rx = new mmap_rx(std::string(of_port_state->name), 2 * block_size, n_blocks, frame_size);
		}
		if(!tx){
			ROFL_DEBUG_VERBOSE(FWD_MOD_NAME"[mmap:%s] generating a new mmap_tx for TX\n",of_port_state->name);
			tx = new mmap_tx(std::string(of_port_state->name), block_size, n_blocks, frame_size);
		}
	...
	if ((rc = ioctl(sd, SIOCSIFFLAGS, &ifr)) < 0){
		ROFL_DEBUG(FWD_MOD_NAME"[mmap:%s] Unable to bring interface down via ioctl\n",of_port_state->name);
		close(sd);
...
rofl_result_t ioport_mmap::down() {
	...
	if(rx){
		ROFL_DEBUG_VERBOSE(FWD_MOD_NAME"[mmap:%s] destroying mmap_int for RX\n",of_port_state->name);
		delete rx;
		rx = NULL;
	}
	if(tx){
		ROFL_DEBUG_VERBOSE(FWD_MOD_NAME"[mmap:%s] destroying mmap_int for TX\n",of_port_state->name);
		delete tx;
		tx = NULL;
	}
