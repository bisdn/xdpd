...
void nf_scope::post_validate(libconfig::Setting& setting, bool dry_run){
	...
	for(int i=0; i<setting.getLength(); ++i){
		...
		ROFL_DEBUG_VERBOSE(DEFAULT, CONF_PLUGIN_ID "[%s] Found nf interface: %s\n", get_path().c_str(), nf_port_name.c_str());
		//Check for port existance		
		if(port_manager::exists(nf_port_name) == true){
			ROFL_ERR(DEFAULT, CONF_PLUGIN_ID "%s: ERROR the port name '%s' already exists!\n", setting.getPath().c_str(), nf_port_name.c_str());
			throw eConfParseError(); 	
		}

		if( setting[i].exists(TYPE) == false){
			ROFL_ERR(DEFAULT, CONF_PLUGIN_ID "%s: ERROR the port type for port name '%s' is missing or has an invalid type!\n", setting.getPath().c_str(), nf_port_name.c_str());
		}
		...
		try{
			...
			ROFL_ERR(DEFAULT, CONF_PLUGIN_ID "%s: ERROR the port type for port name '%s' is invalid!\n", setting.getPath().c_str(), nf_port_name.c_str());
		}	
		...
		if(type == PORT_TYPE_NF_SHMEM){
			...
			try{
				...
				ROFL_ERR(DEFAULT, CONF_PLUGIN_ID "%s: ERROR mandatory '%s' is missing or is invalid for port name '%s' of type '%s'!\n", setting.getPath().c_str(), NF_NAME, nf_port_name.c_str(), PT_SHMEM);
				throw eConfParseError(); 	
			}
		}
		...
		if(!dry_run){
			...
			try{
				...
				ROFL_ERR(DEFAULT, CONF_PLUGIN_ID "%s: unable to create port '%s'. Unknown error.\n", setting.getPath().c_str(), nf_port_name.c_str());
				throw;
